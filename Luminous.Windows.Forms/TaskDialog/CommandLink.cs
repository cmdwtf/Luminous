...
namespace Luminous.Windows.Forms
{
	...
	public partial class CommandLink : System.Windows.Forms.Button
	{
		...
		/// </summary>
		[Category("Appearance"), Description("Indicates whether the button should be decorated with the security shield icon."), BrowsableAttribute(true), DefaultValue(false)]
		...
		private readonly Color _pressedBorder;
		private readonly Color _pressedBackground;
		private readonly Color _activeBorderHot1;
		private readonly Color _activeBorderHot2;
		private readonly Color _activeBorderCold;
		private readonly Color _hoveredBorder1;
		private readonly Color _hoveredBorder2;
		private readonly Color _hoveredBorder3;
		private readonly Color _hoveredBackground1;
		private readonly Color _hoveredBackground2;
		private readonly Color _hoveredBackground3;
		...
		private void DrawButtonBackground(Graphics g, Rectangle rectangle, FrameType frameType)
		{
			...
			switch (frameType)
			{
				...
				case FrameType.Pressed:
					...
					{
						using (var p = new Pen(_pressedBorder))
						{
							g.DrawPath(p, bw);
						}
				...
				case FrameType.Hovered:
					...
					using (GraphicsPath bw = CreateRoundRectangle(rect, 2))
					{
						...
						else
						{
							...
							using (var b = new LinearGradientBrush(rect1, _hoveredBackground1, _hoveredBackground2, LinearGradientMode.Vertical))
							{
								g.SetClip(rect1);
								g.FillPath(b, bw);
								g.ResetClip();
							}
							using (var b = new LinearGradientBrush(rect2, _hoveredBackground2, _hoveredBackground3, LinearGradientMode.Vertical))
							{
								g.SetClip(rect2);
								g.FillPath(b, bw);
								g.ResetClip();
