...
namespace Luminous.Xml.Serialization
{
	...
	public static class XmlSerializer2
	{
		public static string Serialize<T>(T obj)
			where T : new() => @"<?xml version=""1.0"" encoding=""utf-8""?>" + Environment.NewLine + SerializeWithoutXmlDeclaration(obj);
		...
		private static void ProcessXmlDocument(XmlDocument xdoc) => ProcessElement(0, xdoc.DocumentElement);
		...
		private static void ProcessElement(int level, XmlElement x)
		{
			...
			if (x.ChildNodes == null || x.ChildNodes.Count == 0 || (x.ChildNodes.Count == 1 && !(x.FirstChild is XmlElement)))
			{
				return;
			}

			XmlAttribute a = x.Attributes["xml:space"];
			if (a != null && a.Value == "preserve")
			{
				return;
			}
		...
		public static T Deserialize<T>(string xml)
			where T : new() => (T)Deserialize(xml, typeof(T));
